...
fetch('/beers')
  .then(res => res.json())
  .then(beers => {
    beers.map((beer) => rootElement.insertAdjacentHTML('beforeend', beerComponent(beer.name, beer.price, beer.rating, beer.id)))
      ...
      button.addEventListener('click', (e) => {
        ...
        fetch(`/beers/${beerId}`)
          ...
          .then(beerData => {
            ...
            rootElement.insertAdjacentHTML('afterbegin', beerEditComponent(beerData.name, beerData.price, beerData.rating, beerData.id))
